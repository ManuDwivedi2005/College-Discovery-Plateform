...
app.post("/auth/register", async (req, res) => {
    try {
        ...
        if (userRes.rows.length > 0) {
            ...
            const result = await query(`INSERT INTO users (name, email, password_hash, role, is_verified) VALUES ($1, $2, $3, $4, true) RETURNING id, name, email, role`, [name, email, hash, role]);
            user = result.rows[0];
        }
...
app.post("/qa", authenticateToken, async (req, res) => {
    try {
...
app.post("/qa/:id/answers", authenticateToken, async (req, res) => {
    try {
...
app.delete("/qa/:id", authenticateToken, async (req, res) => {
    try {
        const id = Number(req.params.id);
        const qRes = await query(`SELECT user_id FROM questions WHERE id = $1`, [id]);
        if (qRes.rows.length === 0)
            return res.status(404).json({ error: "Not found" });
        if (qRes.rows[0].user_id !== req.user.id)
...
app.delete("/qa/answers/:id", authenticateToken, async (req, res) => {
    try {
        const id = Number(req.params.id);
...
// Reviews Route
app.post("/colleges/:slug/reviews", authenticateToken, async (req, res) => {
    try {
        const { slug } = req.params;
        ...
        const r = await query("INSERT INTO reviews (college_id, user_id, author, role, rating, year, comment) VALUES ($1, $2, $3, (SELECT role FROM users WHERE id = $2), $4, $5, $6) RETURNING id", [collegeId, req.user.id, req.user.name, rating, new Date().getFullYear(), comment]);
        res.json({ data: r.rows[0] });
    }
    catch (err) {
        res.status(400).json({ error: err.message || "Invalid input" });
...
app.delete("/colleges/:slug/reviews/:id", authenticateToken, async (req, res) => {
    try {
        const id = Number(req.params.id);
        const rRes = await query("SELECT user_id FROM reviews WHERE id = $1", [id]);
        if (rRes.rows.length === 0)
            return res.status(404).json({ error: "Not found" });
        if (rRes.rows[0].user_id !== req.user.id)
            return res.status(403).json({ error: "Forbidden" });
...
app.post("/colleges", authenticateToken, async (req, res) => {
    try {
        ...
        const r = await query("INSERT INTO colleges (slug, name, location, state, ownership, fees_annual, rating, placement_rate, median_package_lpa, highest_package_lpa, established_year, overview, exams, website_url) VALUES ($1, $2, $3, $4, $5, $6, $7, 85, 5, 10, 2000, 'Pending full details update', $8, $9) RETURNING id, slug", [slug, name, location, state, ownership, feesAnnual, rating, exams, websiteUrl]);
        res.json({ data: r.rows[0] });
    }
    catch (err) {
        res.status(400).json({ error: err.message || "Invalid input" });
