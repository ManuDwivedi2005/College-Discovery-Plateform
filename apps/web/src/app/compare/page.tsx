...
import { formatINR, type College } from "@college/shared";
import { fetchComparison, fetchCollegeOptions } from "@/lib/college-api";
...
export default async function ComparePage(props: Props) {
  ...
  const optionsRes = await fetchCollegeOptions();
  const dropdownColleges = optionsRes.data || [];

  let selected: College[] = [];
  if (slugs.length >= 2) {
    const compareRes = await fetchComparison(slugs as string[]);
    selected = (compareRes.data || []).sort((a, b) => slugs.indexOf(a.slug) - slugs.indexOf(b.slug));
  }
    ...
    <main className="min-h-screen bg-slate-50">
      <section className="mx-auto w-full max-w-7xl px-6 py-8 sm:px-10 lg:px-12">
        <div className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-[0_20px_80px_rgba(15,23,42,0.06)] lg:p-8">
          <p className="text-sm font-semibold uppercase tracking-[0.3em] text-amber-600">Compare colleges</p>
          ...
          <form className="mt-8 grid gap-4 lg:grid-cols-[1fr_1fr_1fr_auto]" method="get">
            {[1, 2, 3].map((index) => (
              <label key={index} className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3">
                <div className="text-xs uppercase tracking-[0.2em] text-slate-400">College {index}</div>
                <select name={`slug${index}`} defaultValue={slugs[index - 1] ?? ""} className="mt-1 w-full bg-transparent text-sm outline-none">
            ...
            <button className="rounded-2xl bg-slate-950 px-5 py-3 text-sm font-semibold text-white shadow-lg transition hover:bg-slate-800">Compare</button>
          </form>
        </div>
        ...
        {selected.length >= 2 ? (
                ...
                <thead>
                  <tr className="bg-slate-950 text-white">
                    ...
                    {selected.map((college) => (
                      <th key={college.slug} className="px-6 py-4 text-sm font-semibold">{college.name}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {metrics.map((metric) => (
                    <tr key={metric.label} className="border-t border-slate-200 even:bg-slate-50">
                      <th className="sticky left-0 z-10 bg-inherit px-6 py-4 text-sm font-semibold text-slate-700">{metric.label}</th>
                      {selected.map((college) => (
                        <td key={college.slug} className="px-6 py-4 text-sm text-slate-600">{metric.resolve(college)}</td>
                      ))}
                    </tr>
                  ))}
                </tbody>
          ...
          <div className="mt-8 rounded-[1.75rem] border border-dashed border-slate-300 bg-white p-8 text-sm text-slate-600">Select at least two colleges to unlock the comparison table.</div>
        )}
      </section>
    </main>
  );
